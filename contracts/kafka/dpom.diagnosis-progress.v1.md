...
- Delivery: at-least-once；SSE 与 Kafka 都读取同一个 DPOMBase persisted progress log
...
- 建议 retention 24 小时、cleanup policy `delete`；Portal SSE replay window 与该 retention 独立，直接读取 DPOMBase 数据库。
- 可重试 broker/DB 故障有界重试；安全/契约/顺序冲突写入 SRE durable quarantine 后 ack，不使用 broker DLT 代替审计。
- topic ACL：DPOMBase producer-only，SRE consumer-only；Portal 通过受认证的 DPOMBase SSE/REST，不消费 Kafka。
